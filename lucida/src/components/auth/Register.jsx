...
export default function Register() {
  ...
  const handleRegister = (e) => {
    ...
    // LocalStorage Mock DB Save
    const user = { name, email, password };
    localStorage.setItem('lucida_user_auth', JSON.stringify(user));
    ...
    <div className="min-h-screen bg-[#05060f] flex flex-col justify-center items-center px-4 sm:px-6 font-['Plus_Jakarta_Sans',sans-serif] relative overflow-hidden">
      ...
      <div className="w-full max-w-sm sm:max-w-md lg:max-w-lg bg-[rgba(255,255,255,0.02)] backdrop-blur-[40px] border border-[rgba(255,255,255,0.08)] p-8 sm:p-10 lg:p-14 rounded-[32px] sm:rounded-[40px] shadow-[0_40px_80px_rgba(0,0,0,0.6)] relative z-10 my-16">
        <div className="text-center sm:text-left mb-10">
          <h2 className="text-4xl sm:text-5xl font-black tracking-tighter text-white mb-3">Deploy Engine</h2>
          <p className="text-base sm:text-lg text-white/40 font-medium">Create your optimized reading profile.</p>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-6">
          <div>
            <label className="block text-xs sm:text-sm uppercase tracking-widest font-black text-indigo-400 mb-2.5">Full Name</label>
            <input 
              type="text" 
              value={name}
        ...
        <p className="mt-10 text-center text-base text-white/40 font-medium">
          Already have an account? <Link to="/login" className="text-indigo-400 hover:text-white transition-colors">Log In.</Link>
        </p>
      </div>
    </div>
  );
